...
int relayPins[3] = {13, 12, 11}; // Inlet, Rinse, Pump
int productFlowPin = 2; // Product flow sensor
int wasteFlowPin = 3; // Waste flow sensor
...
void setup() {
  ...
  for (int i = 0; i < NUM_BUTTONS; i++) {
    ...
    tft.setCursor(i * buttonWidth + 10, buttonTop + 10);
  ...
  if (cts.touched()) {
    ...
    for (int i = 0; i < 3; i++) {
      if (p.x >= i * buttonWidth && p.x < (i+1) * buttonWidth &&
        ...
        // The touch event was within the area of button i. Toggle the state of relay i.
        ...
        tft.fillRect(i * buttonWidth, tft.height() / 2, buttonWidth, buttonHeight, color);
      }
    }
  }
}
